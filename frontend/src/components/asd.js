...
const CargaVisitas = () => {
    const [modalFoto, setModalFoto] = useState(null); // Estado para la foto en el modal
    const [isModalOpen, setIsModalOpen] = useState(false); // Estado para controlar si el modal está abierto
    const handleViewPhotoSancionada = (foto) => {
        setModalFoto(foto);
    ...
    const closeModal = () => {
        ...
        setModalFoto(null); // Limpia la imagen cuando el modal se cierra
    };

    const handleSubirFotoProhibida = (index) => {
        const nuevaFoto = prompt("Sube una nueva foto:"); // Aquí puedes implementar un input real para seleccionar la foto
        if (nuevaFoto) {
            setHistorialVisitasProhibidas((prevHistorial) =>
                prevHistorial.map((visita, i) => (i === index ? { ...visita, foto: nuevaFoto } : visita))
            );
        }
    };
    const handleSubirFotoSancionada = (index) => {
        const nuevaFoto = prompt("Sube una nueva foto:"); // Aquí puedes implementar un input real para seleccionar la foto
        if (nuevaFoto) {
            setHistorialVisitasSancionadas((prevHistorial) =>
                prevHistorial.map((visita, i) => (i === index ? { ...visita, foto: nuevaFoto } : visita))
            );
        }
    };

    const handleGuardarVisitaProhibida = () => {
        const nuevaEntradaHistorial = {
            ...nuevaVisitaProhibida,
            foto: nuevaVisitaProhibida.foto || null,
            fechaCarga: new Date().toLocaleString(),
        };
        setHistorialVisitasProhibidas((prevHistorial) => [...prevHistorial, nuevaEntradaHistorial]);
    ...
    const handleGuardarVisitaSancionada = () => {
        const nuevaEntradaHistorial = {
            ...nuevaVisitaSancionada,
            foto: nuevaVisitaSancionada.foto || null,
            fechaCarga: new Date().toLocaleString(),
        };
        setHistorialVisitasSancionadas((prevHistorial) => [...prevHistorial, nuevaEntradaHistorial]);
    ...
    const handleViewPhotoProhibida = (foto) => {
        // Abre un modal con la foto en tamaño grande
        setModalFoto(foto); // Aquí `setModalFoto` es una función que muestra la imagen en un modal
    };
    ...
    const handlePhotoChangeProhibida = (e) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                setNuevaVisitaProhibida((prevState) => ({
                    ...prevState,
                    foto: reader.result
                }));
            };
    ...
    const [fotoSancionada, setFotoSancionada] = useState(null);
    ...
    const validateFieldsSancionada = () => {
        const errors = {};
        if (!nuevaVisitaSancionada.nombre) errors.nombre = 'Nombre/s y Apellido/s es obligatorio.';
        if (!nuevaVisitaSancionada.dni) errors.dni = 'DNI es obligatorio.';
        if (!nuevaVisitaSancionada.relacion) errors.relacion = 'Relación con el interno es obligatorio.';
        if (!nuevaVisitaSancionada.motivo) errors.motivo = 'Motivo de la sanción es obligatorio.';
        setErrorsSancionada(errors);
        return Object.keys(errors).length === 0;
    };
    const handleAddSancionada = () => {
        if (validateFieldsSancionada()) {
            const nuevaVisita = {
                ...nuevaVisitaSancionada,
                foto: fotoSancionada,
                fechaCarga: new Date().toLocaleString()
            };
            setHistorialVisitasSancionadas([...historialVisitasSancionadas, nuevaVisita]);
            setNuevaVisitaSancionada({
                foto: '',
                nombre: '',
                dni: '',
            ...
            setFotoSancionada(null);
        }
    };
    const handleUploadPhotoSancionada = () => {
        const file = fileInputRefSancionada.current.files[0];
        if (file) {
            const reader = new FileReader();
            reader.onloadend = () => {
                setFotoSancionada(reader.result);
            };
            reader.readAsDataURL(file);
        }
    };
    ...
    const [fotoProhibida, setFotoProhibida] = useState(null);
    const fileInputRefProhibida = useRef(null);
    ...
    const validateFieldsProhibida = () => {
        const errors = {};
        if (!nuevaVisitaProhibida.nombre) errors.nombre = 'Nombre/s y Apellido/s es obligatorio.';
        if (!nuevaVisitaProhibida.dni) errors.dni = 'DNI es obligatorio.';
        if (!nuevaVisitaProhibida.relacion) errors.relacion = 'Relación con el interno es obligatorio.';
        if (!nuevaVisitaProhibida.motivo) errors.motivo = 'Motivo de la prohibición es obligatorio.';
        setErrorsProhibida(errors);
        return Object.keys(errors).length === 0;
    };
    const handleAddProhibida = () => {
        if (validateFieldsProhibida()) {
            const nuevaVisita = {
                ...nuevaVisitaProhibida,
                foto: fotoProhibida,
                fechaCarga: new Date().toLocaleString()
            };
            setHistorialVisitasProhibidas([...historialVisitasProhibidas, nuevaVisita]);
            setNuevaVisitaProhibida({
                foto: '',
                nombre: '',
                dni: '',
            ...
            setFotoProhibida(null);
        }
    };
    const handleUploadPhotoProhibida = () => {
        const file = fileInputRefProhibida.current.files[0];
        if (file) {
            const reader = new FileReader();
            reader.onloadend = () => {
                setFotoProhibida(reader.result);
            };
            reader.readAsDataURL(file);
        }
    };
